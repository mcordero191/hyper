...
def keograms(filename,
    ...
    tmin = nn.lb[0] + t0
    if trange is None: tmax = nn.ub[0] #- t0
    else: tmax = tmin+trange
    
    if tstep is None: tstep = (tmax - tmin)/(24*6)
    if zrange is None: zrange = (nn.ub[1] - nn.lb[1])*1e-3
...
if __name__ == '__main__':
    ...
    parser.add_argument('-s', '--subfolder', dest='subfolder', default="SpaceX/hyper24", help='subfolder where the neural network model is stored')
    ...
    df_hari = None #read_H5File(filename)
    ...
    filenames = glob.glob(path_PINN +'/*/h*_i???.h5')
