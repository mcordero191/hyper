...
def main():
    ...
    IN="/Users/radar/Data/IAP/SIMONe/Germany/IAPData/hyper/winds"
    OUT="/Users/radar/Data/IAP/SIMONe/Germany/IAPData/hyper/plots"
    DAY="20240701"
    ALTS="85,90,95"

    p = argparse.ArgumentParser(description="Wind products + continuity diagnostics from DAILY FILES (u,v,w).")

    p.add_argument("--input-dir", default=IN, help="Directory containing daily wind files")
    p.add_argument("--outdir", default=OUT, help="Output directory for plots")
    ...
    p.add_argument("--map-alts", type=str, default=ALTS, help="Comma-separated altitude list in km")
    p.add_argument("--map-day", type=str, default=DAY, help="YYYYMMDD for single day selection (default first day)")
    ...
    p.add_argument("--diag-alts", type=str, default=ALTS, help="Comma-separated altitudes (km) for diagnostic maps")
    p.add_argument("--diag-day", type=str, default=DAY, help="YYYYMMDD for diagnostic maps (default first day)")
    ...
    if int(args.make_diagnostics) == 1:
        ...
        alt_list = parse_float_list(args.diag_alts)
        day_indices = select_day_indices(prod["t"], args.diag_day, all_days=False)
        di = day_indices[0]
        day_str = time_to_yyyymmdd(prod["t"][di])
        proj = args.map_projection
